...
public class ProjectDataProcessJobControl {
	...
	public static void resultMove(Configuration conf, Map<String, String[]> pathMap, boolean isUpdate){
		...
		HdfsUtil.mvFile(conf, pathMap.get("step5")[pathMap.get("step5").length-1] + "/part-r-00000", fs + "/user/mysqlOut/project_all_expert/add_project_all_expert" + currentDate, LOG);
		HdfsUtil.mvFile(conf, pathMap.get("step6")[pathMap.get("step6").length-1] + "/part-r-00000", fs + "/user/mysqlOut/project/add_project" + currentDate, LOG);
...
//		conf.set("mapreduce.job.jar","E:\\AutoDataProcess\\AutoProject\\ProjectDataProcessJobControl.jar");
		
		//不同网页爬取的数据预处理方法不同
		Job preJob = JobFactor.createJob("preProject", conf, ProjectDataProcessJobControl.class, PretreatmentNsfc.Step1JQYQMap.class, 
				PretreatmentNsfc.Step1JQYQReduce.class, Text.class, Text.class, Text.class, NullWritable.class, pathMap.get("preProject"));
